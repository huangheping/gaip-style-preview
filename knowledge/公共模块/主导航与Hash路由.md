...
- 所有注册表标记为virtual的频道共用`learning-nav.js`的选中态保护：按当前Hash和直属`data-gaip-channel`定位当前菜单，监听class变化并幂等清除归属外的旧选中态；叶子同步选中/ARIA，组内子项与展开状态仍由各自控制器管理。菜单未挂载或频道未登记时不接管。不仅监听childList，不清除真实`ant-menu-item-active`或禁用hover。回归：`scripts/test-virtual-nav-state.cjs`覆盖资讯、学习、财富三子页、配置三子页与普通频道返回；学习专项回归继续保留（均为DOM模拟，不等同真实鼠标验收）。
