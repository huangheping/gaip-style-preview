(function () {
  ...
  var entries = [
    ...
    { id: 'agent-session-rename', title: '修改会话名称', channel: 'AI Agent 子页面', type: 'modal', status: 'ready', height: 820, previewMode: 'route-trigger', route: '/agents/riskAccess', source: '登录.html#/agents/riskAccess → 更多操作 → 修改名称' },
    ...
    { id: 'clues-terminal', title: '标记转化 / 标记关闭', channel: '线索中心 · 详情', type: 'modal', status: 'ready', height: 900, previewMode: 'route-trigger', route: '/clues', source: '登录.html#/clues → 详情 → 转化' },
    ...
    { id: 'product-expert', title: '联系产品专家', channel: '产品中心 · 产品列表', type: 'modal', status: 'ready', height: 900, previewMode: 'route-trigger', route: '/product', source: '登录.html#/product → 联系产品专家' },
  ...
  window.__GAIP_MODAL_SOURCE_CATALOG__ = {
    list: entries,
    ready: entries.filter(function (entry) { return entry.status === 'ready' && entry.type !== 'drawer'; }),
    pending: entries.filter(function (entry) { return entry.status === 'pending' && entry.type !== 'drawer'; }),
    excluded: entries.filter(function (entry) { return entry.status === 'excluded'; }),
    excludedDrawers: entries.filter(function (entry) { return entry.status === 'excluded' && entry.type === 'drawer'; }),
    excludedOther: entries.filter(function (entry) { return entry.status === 'excluded' && entry.type !== 'drawer'; })
  };
}());
