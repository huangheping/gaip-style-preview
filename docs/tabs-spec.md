...
切换当前选项只更新选中状态和下方内容，不替换或临时移除 Tab 容器/按钮。否则新节点直接呈现最终下划线宽度，虽有相同 CSS，动画也不会播放。离开频道、销毁页面时才释放实例；不要为恢复动画增加页面专用参数或强制重挂载。
...
- `test-learning-study-browser.cjs`：真实学情控制器的 Tab 中间帧、稳定实例、快速反向切换及原业务交互回归。
- `test-config-organization-shared.cjs`：真实组织控制器的渠道切换/键盘焦点、稳定 Tab、无重复头部控件（DOM）。
- `test-config-tabs-animation-browser.cjs`：真实组织控制器中间帧、稳定实例、快速反向切换、键盘及树内容更新。三个浏览器测试复用 `tabs-animation-check.cjs` 验证实际宽度变化，不仅检查最终样式；产品采用原结构/原类切换的独立夹具，非完整 React 页面验收。
