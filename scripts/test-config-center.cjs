...
async function main() {
  ...
  assert.match(configContentCss, /gaip-bulk-result-actions \.gaip-bulk-button\.is-link\s*\{[^}]*align-self:\s*center;[^}]*padding:\s*0;[^}]*text-align:\s*center;[^}]*text-decoration:\s*underline/);
  assert.match(configContentCss, /gaip-bulk-result-card\s*\{[^}]*border:[^}]*background:\s*#fff/);
  ...
  assert.equal(one('.gaip-organization-sidebar > .gaip-channel-slot > .gaip-channel-picker').querySelectorAll('[data-config-channel]').length, 6, 'channel picker preserves all channels');
  assert.equal(d.querySelector('.gaip-config-original .tabs___U1Hwt'), null, 'no competing horizontal channel tabs');
  const channelTrigger = one('[data-channel-trigger]');
  assert.ok(one('[data-channel-panel]').classList.contains('gaip-filter-bar__popup'));
  assert.ok(one('[data-channel-search]').classList.contains('gaip-filter-bar__input'));
  assert.equal(d.querySelectorAll('.gaip-channel-option.gaip-filter-bar__option').length, 6);
  assert.equal(d.querySelector('.gaip-channel-check'), null, 'selected check belongs to the shared option style');
  assert.equal(one('[data-channel-panel]').hidden, true);
  channelTrigger.click();
  assert.equal(channelTrigger.getAttribute('aria-expanded'), 'true');
  assert.equal(d.activeElement, one('[data-channel-search]'));
  const channelSearch = one('[data-channel-search]');
  channelSearch.value = 'Glory'; channelSearch.dispatchEvent(new w.Event('input', { bubbles: true }));
  assert.deepEqual(Array.from(d.querySelectorAll('[data-config-channel]')).filter(el => !el.hidden).map(el => el.dataset.configChannel), ['1']);
  channelSearch.dispatchEvent(new w.KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true }));
  assert.equal(d.activeElement, one('[data-config-channel="1"]'));
  d.activeElement.click();
  assert.equal(one('[data-channel-current]').textContent, 'Glory品牌顾问');
  assert.equal(one('[data-channel-panel]').hidden, true);
  assert.equal(d.activeElement, channelTrigger, 'selection restores focus without replacing the trigger');
  channelTrigger.click();
  channelSearch.value = '没有的渠道'; channelSearch.dispatchEvent(new w.Event('input', { bubbles: true }));
  assert.equal(one('[data-channel-empty]').hidden, false);
  channelSearch.dispatchEvent(new w.KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
  assert.equal(one('[data-channel-current]').textContent, 'Glory品牌顾问', 'empty search does not change channel');
  channelSearch.dispatchEvent(new w.KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
  assert.equal(channelTrigger.getAttribute('aria-expanded'), 'false');
  channelTrigger.click();
  d.body.dispatchEvent(new w.Event('pointerdown', { bubbles: true }));
  assert.equal(one('[data-channel-panel]').hidden, true, 'outside press closes picker');
  channelTrigger.click();
  channelSearch.value = '薄荷'; channelSearch.dispatchEvent(new w.Event('input', { bubbles: true }));
  channelSearch.dispatchEvent(new w.KeyboardEvent('keydown', { key: 'Enter', bubbles: true }));
  assert.equal(one('[data-channel-current]').textContent, '薄荷经纪人');
  ...
  assert.equal(toolbarAdminDialog.querySelector('.footerTip___Iu6Of'), null);
  assert.equal(toolbarAdminDialog.querySelectorAll('.gaip-admin-info-icon').length, 4);
  assert.deepEqual(Array.from(toolbarAdminDialog.querySelectorAll('[data-admin-role-panel] .adminRoleHeading___fR73p p'), el => el.textContent), ['可管理当前节点及下级组织。', '可不设置，最多 1 人。', '仅超级管理员可配置。', '负责可见范围内的线索跟进，由管理员或线索管理员配置。线索管理员仅可设置线索跟进人，不可取消线索管理员身份。']);
  assert.deepEqual(Array.from(toolbarAdminDialog.querySelectorAll('[data-admin-role-tab] strong'), el => el.textContent), ['组织架构管理员（人管）', '上级佣金归属人（佣金）', '线索管理员（线索）', '线索跟进人']);
  ...
  assert.match(toolbarAdminDialog.querySelector('[data-admin-role-panel="clue-admin"]').textContent, /仅超级管理员可配置/);
  assert.equal(toolbarAdminDialog.querySelector('.adminIntro___gP4Jq p').textContent, '请选择当前组织节点下的管理员。线索管理员与线索跟进人不可为同一成员。');
  assert.ok(toolbarAdminDialog.querySelector('[data-admin-role-panel="clue-follower"] .adminRoleHeading___fR73p p br'));
  ...
  assert.equal(memberSearch.placeholder, '跨渠道搜索成员');
  assert.equal(one('[data-member-results]').hidden, false);
  assert.ok(one('[data-member-results]').textContent.includes('薄荷经纪人 /'), 'results include the organization path');
  assert.ok(d.querySelectorAll('tbody tr').length > 1, 'global search does not filter or change node-scoped operations');
  memberSearch.value = '渠道'; memberSearch.dispatchEvent(new w.Event('input', { bubbles: true }));
  assert.ok(d.querySelectorAll('[data-member-result]').length > 0, 'cross-channel mock members are searchable');
  ...
  memberSearch.focus();
  memberSearch.value = 'Glory'; memberSearch.dispatchEvent(new w.Event('input', { bubbles: true }));
  const crossChannelResult = one('[data-member-result]');
  const searchedMemberId = crossChannelResult.dataset.memberResult;
  assert.ok(crossChannelResult.textContent.includes('Glory品牌顾问'));
  memberSearch.dispatchEvent(new w.KeyboardEvent('keydown', { key: 'ArrowDown', bubbles: true }));
  assert.equal(d.activeElement, crossChannelResult);
  crossChannelResult.click();
  assert.equal(one('[data-channel-current]').textContent, 'Glory品牌顾问');
  assert.equal(one('input[aria-label="搜索成员"]').value, '');
  assert.equal(one('[data-member-results]').hidden, true);
  assert.equal(one('tr.is-search-target').dataset.memberId, searchedMemberId);
  assert.ok(one('.ant-tree-treenode-selected').hidden === false, 'result reveals its selected node');
  assert.equal(d.activeElement.dataset.edit, searchedMemberId);
  click('[data-config-channel="0"]');
  const emptySearch = one('input[aria-label="搜索成员"]');
  emptySearch.value = 'NO_SUCH_MEMBER'; emptySearch.dispatchEvent(new w.Event('input', { bubbles: true }));
  assert.equal(d.querySelectorAll('[data-member-result]').length, 0);
  assert.ok(one('[data-member-results]').textContent.includes('未找到匹配成员'));
  emptySearch.dispatchEvent(new w.KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
  assert.equal(one('[data-member-results]').hidden, true);
  click('[data-config-clear]');
  ...
  assert.equal(one('#isCommissionOwner').disabled, false, 'unconfigured node allows a commission owner');
  assert.equal(d.querySelector('.gaip-member-commission-hint'), null, 'unconfigured node has no existing-owner hint');
  ...
  // Product §5.3: editing the owner cannot revoke ownership; siblings cannot take it.
  select('all'); // Aggregated lists must still use the member's actual node.
  click('[data-edit="3"]');
  const commissionOwnerName = one('#userName').value;
  assert.equal(one('#isCommissionOwner').checked, true);
  assert.equal(one('#isCommissionOwner').disabled, true, 'the owner is also locked in the member editor');
  assert.equal(one('.gaip-member-commission-hint').textContent, '当前该节点已有佣金管理员：' + commissionOwnerName + '，不可多选。');
  assert.equal(one('.gaip-member-commission-owner-name').textContent, commissionOwnerName, 'only the actual owner name receives the green text style');
  assert.equal(one('.gaip-member-commission-hint').previousElementSibling.classList.contains('memberRoleModule___W8rjP'), true);
  assert.equal(one('.gaip-member-commission-hint').parentElement, one('.gaip-member-role-group'), 'hint and administrator row share a spacing group');
  assert.equal(one('#isCommissionOwner').getAttribute('aria-describedby'), one('.gaip-member-commission-hint').id);
  click('#isCommissionOwner');
  assert.equal(one('#isCommissionOwner').checked, true, 'clicking the disabled owner does not uncheck it');
  one('#isCommissionOwner').disabled = false;
  one('#isCommissionOwner').checked = false;
  click('[data-editor-save]');
  click('[data-edit="3"]');
  assert.equal(one('#isCommissionOwner').checked, true, 'save preserves ownership even if the control is altered');
  click('[data-editor-cancel]');
  click('[data-edit="6"]');
  assert.equal(one('#isCommissionOwner').checked, false);
  assert.equal(one('#isCommissionOwner').disabled, true, 'another member on the same node cannot be selected');
  assert.ok(one('.gaip-member-commission-hint').textContent.includes(commissionOwnerName));
  one('#isCommissionOwner').disabled = false;
  one('#isCommissionOwner').checked = true;
  click('[data-editor-save]');
  click('[data-edit="6"]');
  assert.equal(one('#isCommissionOwner').checked, false, 'save cannot replace an existing owner');
  click('[data-editor-cancel]');
  select('mock-level-3-east');
  ...
  assert.ok(one('[data-bulk-back]').classList.contains('is-secondary'), 'previous step keeps the original outlined action style');
  assert.ok(one('[data-bulk-download-fail]').classList.contains('is-secondary'), 'failure download keeps the original outlined action style');
  assert.ok(one('[data-bulk-download-fail] .gaip-bulk-result-action-icon svg'), 'validation failure download reuses the step-three download SVG');
  assert.equal(one('[data-bulk-footer]').firstElementChild, one('.gaip-bulk-footer-leading'), 'failure download has a leading group outside button ordering');
  assert.equal(one('.gaip-bulk-footer-leading').firstElementChild, one('[data-bulk-download-fail]'));
  assert.equal(one('[data-bulk-back]').nextElementSibling, one('[data-bulk-confirm]'), 'previous and confirm remain adjacent');
  assert.equal(one('.gaip-bulk-footer-leading').nextElementSibling, one('.gaip-bulk-footer-actions'), 'left and right groups are siblings');
  // Include the shared form adapter and its actual CSS order rules: DOM order alone missed this regression.
  w.__GAIP_MODAL_COMPONENT__.adoptForm(bulkDialog);
  const footerRules = ['gaip-bulk-modal-footer', 'gaip-bulk-footer-leading', 'gaip-bulk-footer-actions'].map(cls => {
  ...
  assert.equal(layoutStyle('.gaip-bulk-footer-actions').order || '0', '0', 'right group keeps natural order');
  assert.equal(layoutStyle('.gaip-bulk-footer-leading').marginRight, 'auto', 'leading group absorbs remaining space');
  assert.equal(layoutStyle('.gaip-bulk-footer-actions').marginLeft, 'auto', 'right group stays against the right edge');
  assert.equal(layoutStyle('[data-bulk-back]').order, '1');
  assert.equal(layoutStyle('[data-bulk-confirm]').order, '2');
  ...
  assert.ok(one('.gaip-bulk-result-icon').classList.contains('is-partial'));
  assert.ok(one('.gaip-bulk-result-icon .gaip-bulk-warning'), 'partial failure uses a warning icon instead of a success check');
  const resultCopy = one('.gaip-bulk-result > p');
  const resultActions = one('.gaip-bulk-result-actions');
  const resultCard = one('.gaip-bulk-result-card');
  assert.equal(resultCopy.nextElementSibling, resultActions, 'result actions sit directly below the completion copy');
  assert.equal(resultActions.children.length, 2, 'result actions contain the two requested controls');
  assert.equal(resultActions.firstElementChild, one('[data-bulk-download-fail]'));
  assert.equal(resultActions.lastElementChild, one('[data-bulk-again]'));
  assert.ok(one('[data-bulk-download-fail]').classList.contains('is-primary'), 'failure download becomes the primary recovery action');
  assert.ok(one('[data-bulk-download-fail] .gaip-bulk-result-action-icon svg'), 'failure download uses the existing download SVG');
  assert.ok(one('[data-bulk-again]').classList.contains('is-link'), 'continue import becomes the underlined text action');
  assert.ok(resultCard.contains(one('.gaip-bulk-result-target')), 'target is consolidated into the result detail card');
  assert.ok(resultCard.contains(one('.gaip-bulk-summary-grid')), 'summary is consolidated into the result detail card');
  assert.ok(resultCard.contains(one('.gaip-bulk-result-note')), 'result guidance is consolidated into the result detail card');
  const resultPreviewControl = one('[data-bulk-result-preview-control]');
  assert.equal(resultPreviewControl.parentElement, bulkDialog, 'local result switch is outside the white modal content');
  assert.equal(resultPreviewControl.previousElementSibling, one('.gaip-bulk-import-dialog > .ant-modal-content'));
  assert.equal(resultPreviewControl.hidden, false, 'local result switch is visible on step three');
  assert.equal(one('[data-bulk-footer]').children.length, 0, 'step three no longer duplicates actions in the fixed footer');
  assert.ok(one('[data-bulk-footer]').classList.contains('is-result-step'));
  assert.equal(one('[data-bulk-result-preview="partial"]').getAttribute('aria-pressed'), 'true');
  click('[data-bulk-result-preview="success"]');
  assert.equal(one('[data-bulk-result-preview="success"]').getAttribute('aria-pressed'), 'true', 'local preview can switch to the all-success result');
  assert.equal(one('.gaip-bulk-result h3').textContent, '批量导入已完成');
  assert.ok(one('.gaip-bulk-result-icon').classList.contains('is-success'));
  assert.ok(one('.gaip-bulk-result-icon .gaip-bulk-check'), 'all-success result keeps the success check');
  assert.match(one('.gaip-bulk-result > p').textContent, /全部成员已成功导入/);
  assert.equal(bulkDialog.querySelector('[data-bulk-download-fail]'), null, 'all-success result does not show a failure download action');
  assert.equal(one('.gaip-bulk-result-actions').children.length, 1);
