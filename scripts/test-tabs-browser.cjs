...
(async()=>{const browser=await chromium.launch({headless:true,...(process.env.CHROME_PATH?{executablePath:process.env.CHROME_PATH}:{})});try{
 ...
 assert.equal(styles.length,3);assert.deepEqual(styles[0],styles[1]);assert.deepEqual(styles[1],styles[2]);assert.equal(styles[0].width,'71px');assert.equal(styles[0].transition,'0s');assert.equal(styles[0].weight,'700');assert.equal(styles[0].color,'rgb(47, 54, 64)');
 ...
 assert.deepEqual(await p.locator('[data-gaip-tabs] > [aria-selected="true"]').evaluateAll(xs=>xs.map(n=>({color:getComputedStyle(n).color,padding:getComputedStyle(n).padding,line:getComputedStyle(n,'::after').width}))),Array(3).fill({color:'rgb(47, 54, 64)',padding:'0px 12px',line:'71px'}));
 const checkWeights=async()=>assert.equal(await p.locator('[data-gaip-tab]').evaluateAll(xs=>xs.every(n=>{const expected=n.getAttribute('aria-selected')==='true'?'700':'400';return [n,...n.querySelectorAll('span')].every(el=>getComputedStyle(el).fontWeight===(el.closest('.gaip-tabs-quantity, .tabBtn___Au692 > span:has(> .val___uBwax)')?'400':expected));})),true,'selected labels bold; shared quantity parentheses and digits regular');
 await checkWeights();
 ...
 await checkWeights();
 await p.getByRole('tab',{name:'全部 ( 0 )'}).hover();await checkWeights();
 assert.equal(await p.locator('#demo .gaip-tabs-quantity').textContent(),' ( 0 )','parentheses and number share the regular-weight wrapper');
 assert.equal(await p.locator('#demo .gaip-tabs-quantity').evaluate(n=>getComputedStyle(n).marginInlineStart),'4px','explicit spacing between label and quantity survives flex whitespace collapsing');
 assert.equal(await p.locator('#demo .gaip-tabs-count').evaluate(n=>getComputedStyle(n).color),'rgb(2, 91, 82)','count color unchanged');
