...
- 分支：`main`
- 最近已发布功能提交：`06c9e1a`（仓库维护规则、频道/弹窗 Skill、维护流程与审计报告；见 [[knowledge/变更/2026-09-07-Agent与Skill维护规则优化]]；弹窗关闭状态修复为 `3d344bf`）
...
- 有官方依据的非 Agent/Skill 工程基线首批已本地完成，尚未发布：开发测试依赖锁定、统一检查入口、浏览器验收清单和可选 worktree/setup 接入说明；`npm ci`、`npm test` 本轮通过，不改网站运行时。见 [[docs/official-engineering-baseline]] 与 [[knowledge/变更/当前未发布变更]]。

## 待验证

- 用户已手工确认：切换工作台后刷新仍保留该页、关闭组织架构操作日志后导航可点击、Esc 关闭后可再次打开、登录后及跨频道 AI V1/V2 可切换。证据为本任务用户文字确认，非助手自动化；详见 [[knowledge/变更/当前未发布变更#2026-09-07 用户手工回归确认]]。
...
- 其余未发布候选文件见 [[knowledge/变更/当前未发布变更|当前未发布变更]]；它们没有进入正式提交 `3d344bf`、`06c9e1a`。
