...
- 已按 OpenAI 官方指导优化仓库 Agent/Skill：根规则精简，新增频道维护与弹窗维护 Skill，以及按风险验证流程；待校验收尾，尚未发布。报告见 [[docs/agent-skill-audit-2026-09-07]]。

- 当前没有待发布的正式实现；[[knowledge/变更/当前未发布变更|当前未发布变更]] 只保留用户明确不发布的登录视频测试入口及未被正式页面引用的本地试验文件。
