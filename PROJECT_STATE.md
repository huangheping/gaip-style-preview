...
- 分支：`main`
- 最近已发布功能提交：`3d344bf`（修复关闭弹窗透明宿主拦截页面与主导航；见 [[knowledge/变更/2026-09-07-弹窗关闭状态修复]]）
...
- 仓库 Agent 与 Skill 官方指导审计仍在进行；当前没有可发布的实现产物。[[knowledge/变更/当前未发布变更|当前未发布变更]] 还保留用户明确不发布的登录视频测试入口及未被正式页面引用的本地试验文件。
...
- 其余未发布候选文件见 [[knowledge/变更/当前未发布变更|当前未发布变更]]；它们没有进入正式提交 `3d344bf`。
