...
- 当前没有待发布的网站业务或维护实现；[[knowledge/变更/当前未发布变更|当前未发布变更]] 只保留用户明确不发布的登录视频测试入口及本地试验文件。

## 待验证

- 组织架构专属操作日志弹窗的原生 Esc、顶层遮罩与视觉仍需在既有 `file://` 本地预览手工确认；7 列字段、独立 Mock、分页、关闭、焦点恢复代码、与二级日志控制器隔离及生命周期已通过 DOM 回归。
- 本次审计环境中 AI Agent 入口测试未通过：首次 `direct` 通过、`header-first` 失败，复测在 `direct` 提前失败；均为顶栏重建后切换版本实际 1、预期 2。测试与入口源码未被本次审计修改，需单独区分测试时序与业务问题。真实登录、跨频道和视频/WebGL 效果仍需现场验收。
