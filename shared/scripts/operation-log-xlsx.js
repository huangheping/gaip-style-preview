(function () {
  ...
  function build(rows, options) {
    options = options || {};
    var columnEnd = String.fromCharCode(64 + Math.max(1, Math.min(26, rows[0] ? rows[0].length : 8)));
    ...
    return zip({
      ...
      'xl/workbook.xml': '<workbook xmlns="' + ns + '" xmlns:r="' + rel + '"><sheets><sheet name="' + xml(options.sheetName || '操作日志（模拟）') + '" sheetId="1" r:id="rId1"/></sheets></workbook>',
      ...
      'xl/worksheets/sheet1.xml': '<worksheet xmlns="' + ns + '"><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols><col min="1" max="1" width="8" customWidth="1"/><col min="2" max="2" width="23" customWidth="1"/><col min="3" max="3" width="32" customWidth="1"/><col min="4" max="5" width="16" customWidth="1"/><col min="6" max="8" width="50" customWidth="1"/></cols><sheetData>' + data + '</sheetData><autoFilter ref="A1:' + columnEnd + '' + Math.max(1, rows.length) + '"/>' + (options.confidential ? '<headerFooter><oddHeader>&amp;C机密文件</oddHeader><oddFooter>&amp;C机密文件 · &amp;P / &amp;N</oddFooter></headerFooter>' : '') + '</worksheet>'
    });
  }
