(function () {
  ...
  function createGroup() {
    ...
    group.className = 'ant-menu-submenu ant-menu-submenu-inline gaip-main-menu-group gaip-wealth-menu-group';
    group.setAttribute('data-gaip-channel', 'wealth');
    group.setAttribute('role', 'none');
    group.innerHTML =
      '<div class="ant-menu-submenu-title gaip-main-menu-parent gaip-wealth-menu-parent" role="menuitem" tabindex="0" aria-haspopup="true" aria-expanded="false" data-gaip-main-menu-toggle data-wealth-menu-toggle>' +
        '<span class="ant-menu-title-content"><span class="ant-pro-base-menu-inline-item-title gaip-main-menu-title gaip-wealth-menu-title">' +
          '<span class="ant-pro-base-menu-inline-item-icon gaip-main-menu-leading-icon gaip-wealth-menu-icon" aria-hidden="true"><svg viewBox="0 0 18 18" fill="none"><path d="M16.5 10.818V16h-15V2h15v4.677" stroke="currentColor" stroke-width="1.3"/><path d="M17.35 6.65v4.7H15a2.343 2.343 0 0 1-1.662-.688A2.343 2.343 0 0 1 12.65 9c0-.649.263-1.236.688-1.662A2.343 2.343 0 0 1 15 6.65h2.35ZM5.5 15.5v-13" stroke="currentColor" stroke-width="1.3"/></svg></span>' +
          '<span class="ant-pro-base-menu-inline-item-text ant-pro-base-menu-inline-item-text-has-icon gaip-main-menu-label">财富值中心</span>' +
        '</span></span>' +
        '<span class="ant-menu-submenu-arrow gaip-main-menu-caret gaip-wealth-menu-caret" aria-hidden="true">' +
          '<svg class="gaip-main-menu-caret-icon" viewBox="0 0 12 12" fill="none"><path d="m2.5 4.25 3.5 3.5 3.5-3.5" stroke="currentColor" stroke-width="1.35" stroke-linecap="round" stroke-linejoin="round"/></svg>' +
        '</span>' +
      '</div>' +
      '<ul class="ant-menu ant-menu-sub ant-menu-inline gaip-main-menu-submenu gaip-wealth-submenu" role="menu" hidden>' + views.map(function (view) {
  ...
  function bindGroup(group) {
    ...
    group.addEventListener('keydown', function (event) {
      ...
      if (toggle && event.key === 'Escape') {
        event.preventDefault();
        group.setAttribute('data-wealth-menu-open', 'false');
        updateGroup(group);
        toggle.focus();
      }
    });
  ...
  function bindChannelLeaving(menu) {
    ...
    menu.addEventListener('click', function (event) {
      var api;
      if (!isWealthPage() || event.target.closest('.gaip-wealth-menu-group') || event.target.closest('[data-gaip-main-menu-toggle]')) return;
