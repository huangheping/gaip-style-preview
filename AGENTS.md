...
| 视觉样式修改与前端交接 | `docs/visual-change-workflow.md`；只读写 `design-changes/` 下本次页面或共享组件的记录 |
...
- 每轮视觉样式修改将更新记录作为完成条件：首次动手前保存可获得的旧值/依据；收尾把最终参数写入 `design-changes/<页面或共享组件>.json`。仅记录图标、字体/字号/字重/颜色/行高、宽高/尺寸约束、内边距/视觉间距、圆角/边框/背景/阴影；搜索框同样只记样式。位置迁移、导航、交互、权限和接口不进入视觉标注，按产品文档及既有维护流程处理。
- 同一交付基准、元素、状态和属性持续合并，保留首次基准与最新目标；试验中间值不累加成多条交付项。恢复基准时取消对应差异。按 [详细流程](docs/visual-change-workflow.md) 区分线上基准、首次本地值和当前目标，未采集不得猜值。
- 默认只读本次记录和相关源码；完成时核验本轮 diff 与记录，公共样式复核受影响消费者。交付时从记录生成新版页面上的元素标注，旧版只作详情依据；同类一次标注并写范围。不因普通小改重扫全站、重截全部页面或重建历史。
- 样式记录是视觉参数交付的唯一手工维护台账，当前未发布变更和状态文件只留摘要/链接。规则不等于已安装自动监听或标注界面；新增历史基准、整页比较和界面开发按用户实际任务执行。抽离时只复制已有记录到目标目录，不为抽离向来源项目补写记录。
